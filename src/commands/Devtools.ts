...
async function directToggleDevTools() {
    try {
        ...
        if (!config.devtools && !config.$development?.devtools) {
            ...
            window.showInformationMessage(`Nuxt Devtools ${config.devtools.enabled ? 'enabled' : 'disabled'}.`)
        }
        ...
        if (config.devtools) {
            ...
            window.showInformationMessage(`Nuxt Devtools ${isEnabled ? 'enabled' : 'disabled'}.`)
...
async function nuxtConfigWatcher() {
    ...
    if (config.$development) {
        if (config.$development.devtools && typeof config.$development.devtools === 'object') {
    ...
    const tooltip = devtoolsEnabled ? 'Nuxt Devtools: Installed & Enabled' : 'Nuxt Devtools: Installed & Disabled';
    const color = devtoolsEnabled ? '#00DC82' : new ThemeColor('activityBar.inactiveForeground');
